...
Provide detailed security assessment in JSON format:
{
  ...
  "recommended_actions": [
    "IMPORTANT: Provide SPECIFIC, ACTIONABLE recommendations. Each recommendation must include:",
    "- Exact console path (e.g., 'Google Workspace Admin Console > Security > Authentication > 2-Step Verification')",
    "- Specific action steps (not 'review settings' but 'verify Status: Enforced is set, if not click Edit > Enforcement: On')",
    "- Clear scope (e.g., 'for user jordan@domain.com' or 'for all users in /Administrators OU')",
    "- Rationale tied to your analysis (why this action addresses the threat)",
    "- Timeline (e.g., 'immediately', 'within 24 hours', 'at next security review')",
    "",
    "Examples of GOOD recommendations:",
    "- 'Navigate to Google Workspace Admin Console > Directory > Users > [user_email] > Security > 2-Step Verification and verify Status shows Enforced. If user has trusted devices listed, review Last used timestamps and revoke any devices with Last used matching this event timestamp to prevent future MFA bypasses.'",
    "- 'Configure real-time alerting in Admin Console > Security > Alert Center > Manage Rules > Create Rule for event type Suspicious login blocked affecting [user_email]. Set notification to security@domain.com with immediate delivery. This detects future authentication bypass attempts within 5 minutes.'",
    "",
    "Examples of BAD (too vague) recommendations:",
    "- 'Monitor user activity' (WHERE? HOW? FOR HOW LONG?)",
    "- 'Review MFA settings' (WHICH SETTINGS? WHAT TO LOOK FOR?)",
    "- 'Enable security features' (WHICH ONES? HOW?)",
    "",
    "Provide 3-5 specific recommendations appropriate to your risk assessment"
  ],
